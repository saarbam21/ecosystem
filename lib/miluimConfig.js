...
export function computeMiluim(cfg, input) {
  ...
  return {
    ...
    vacationDetail: benefit.vacationDetail,
    fighterWallet: benefit.fighterWallet,
    fighterDetail: benefit.fighterDetail,
...
function computeBenefits(benefits, gk, days, hasChild) {
  ...
  if (vg && days >= num(vv.minDays)) {
    ...
    const from = num(vv.supplementFromDay);
    const to = num(vv.supplementToDay);
    const suppDays = Math.max(0, Math.min(days, to) - (from - 1));
    const raw = base + perDay * suppDays;
    vacationVoucher = Math.min(raw, max);
    vacationDetail = { base, perDay, suppDays, from, to, max, capped: raw > max };
  }

  let fighterWallet = 0;
  let fighterDetail = null;
  ...
  if (fg && Array.isArray(fg.tiers) && fg.tiers.length && days >= num(fw.minDays)) {
    ...
    for (const t of fg.tiers) {
      ...
      if (days <= prev) break;
      const to = Math.min(days, ceil);
      const pay = (to - prev) * num(t.rate);
      bands.push({ from: prev + 1, to, rate: num(t.rate), pay });
      raw += pay;
      prev = ceil;
    }
    const max = num(fg.max);
    fighterWallet = Math.min(raw, max);
    fighterDetail = { bands, max, capped: raw > max };
  }
