...
export function computeMiluim(cfg, input) {
  ...
  // רכיבים שוטפים (מצטברים לפי יום) מול מענקים חד־פעמיים.
  const recurringTotal = reservePay + specialComp + personalExpenses + familyGrant;
  const oneTimeTotal = householdGrant + specialFamilyGrant;
  // סך המענקים הפטורים ממס, והתגמול (החזר שכר) חייב במס בנפרד.
  const grantsTotal =
    specialComp + personalExpenses + familyGrant + householdGrant + specialFamilyGrant;
